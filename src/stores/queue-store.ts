...
export const useQueueStore = create<QueueState>()((set, get) => ({
    ...
    set((state) => {
      ...
      return {
        ...
        run: {
          ...run,
          total: ordinal,
          phase: state.isPaused ? "paused" : run.phase === "idle" ? "idle" : run.phase,
        },
      }
    })
  ...
  remove: (id) =>
    set((state) => {
      ...
      const canShrinkTotal = removed?.status === "pending" && removed.ordinal > state.run.current
      ...
      return {
        ...
        run: {
          ...state.run,
          total: canShrinkTotal ? Math.max(state.run.current, state.run.total - 1) : state.run.total,
        },
      }
    }),
